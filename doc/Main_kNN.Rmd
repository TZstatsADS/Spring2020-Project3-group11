...
dat_test <- dat_test[,-ncol(dat_test)]
label_train <- dat_train[,ncol(dat_train)]
label_test <- dat_test[,ncol(dat_test)]
```
...
```{r final_train}
tm_train=NA
tm_train <- system.time(fit_train <- class::knn(
  train = dat_train[,-which(names(dat_train) == 'emotion_idx')], 
  test = dat_train[,-which(names(dat_test) == 'emotion_idx')], 
  cl = dat_train$emotion_idx,  
  k = par_best))
tm_test <- system.time(fit_test <- class::knn(
  train = dat_train[,-which(names(dat_train) == 'emotion_idx')], 
...
* evaluation
```{r}
accu <- mean(dat_test$emotion_idx == fit_test)
cat("The accuracy of model:", model_labels[which.min(err_cv[,1])], "is", accu*100, "%.\n")

library(caret)
confusionMatrix(fit_test, dat_test$emotion_idx)
```
...
```{r}
library(gbm)
fit.model <- gbm.fit(x = dat_train[,-which(names(dat_train) == 'emotion_idx')],
...
```{r}
pred.prob <- predict(fit.model, newdata=as.data.frame(dat_test[,-6007]),n.trees = 100,type= "response")
error <- sum(pred != dat_test[,6007])/nrow(dat_test)
error
```

```{r}
train_matrix <- xgb.DMatrix(data=data.matrix(dat_train[,-6007]),label=as.numeric(dat_train[,6007])-1)
...
```{r}
pred <- predict(fit.model, newdata=as.matrix(dat_test[-6007]), n.trees = 3)
View(pred)
```

```{r}
error <- sum((pred+1) != dat_test[,6007])/nrow(dat_test)
error
```
